...
def main():
    ...
    uni_datasets = ActionGenerationWithVqaFromLeRobotDataset(
        prev_action_size=6,
        ...
        use_norm=False,
        libero_dataset=True,
        libero_vqa_dataset_path=config.dataset.text_annotation_paths,
        repo_id=".",
        root=config.dataset.dataset_file_paths,
        force_cache_sync=False,
        revision=None,
        download_videos=False,
    )

    uni_dataloader = DataLoader(
        uni_datasets,
        batch_size=config.training.batch_size,
        num_workers=config.training.dataprocess_nums,
        shuffle=True,
        collate_fn=ActionGenerationWithVqaFromLeRobotDataset.collate_fn,
    )
    ...
    def prepare_inputs_and_labels_for_mmu_action(batch, eps=1e-3):
        images, tasks, state_tokens, _, _, action_dims, descriptions = batch
        image_tokens = []
        for imgs in images:
            img_tokens = []
            for img in imgs:
    ...
    model, optimizer, dataloader = accelerator.prepare(model, optimizer, uni_dataloader)
